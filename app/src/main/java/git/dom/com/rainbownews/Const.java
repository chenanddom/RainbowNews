package git.dom.com.rainbownews;

/**
 * 产量类（可以实现方便的改变）
 * Created by Administrator on 2016/8/23 0023.
 */
public class Const {
    //-------------------------------------------------------定义字符串常量-----------------------------------------------------
    public static final String ISUPDATE = "isupdate";
    public static final String ISNIGHTMODE = "isnightmode";
    public static final String ISPRIVATE = "isprivate";
    public static final String WEATHERCODE = "weather";
    public static final String CITYNAMEP = "citynamep";
    public static final String CITYNAMEC = "citynamec";
    public static final String ISNEWSPUSH = "isnewspush";
    public static final String ISACCOUTSAVE = "isaccoutsave";
    public static final String ACCOUTNAME = "accout";
    public static final String ACCOUTPASS = "pass";
    public static final String ISRECORDS = "isrecord";
    public static final String ISCLEARCHACHE = "isclearchache";
    public static final String STRUrl = "http://192.168.0.104:8088/update2.json";
    public static final String MKEY = "f9a6dc0392b9c598afcf80c60048f8ef";
    public static final String HOMENEWS = "http://api.huceo.com/guonei/?key=" + MKEY + "&num=10&page=1";
    public static final String FOREIGNNEWS = "http://api.huceo.com/world/?key=" + MKEY + "&num=10&page=1";
    public static final String SOCIETYNEWS = "http://api.huceo.com/social/?key=" + MKEY + "&num=10&page=1";
    public static final String TECHNEWS = "http://api.huceo.com/keji/?key=" + MKEY + "&num=10&page=1";
    public static final String NEWSDBNAME = "News";
    //----------------------------------定义整型常量量----------------------------------------------------------------------------
    public static final int NEWSDBVERSION = 1;
    public static final int UPDATE_CODE = 1;
    public static final int ERR_CODE = 2;
    public static final int WEATHER_CODE = 3;
    public static final int ENTERY_HOME = 4;
}
